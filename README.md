...
- **URL**: http://43.200.163.20/
- **인프라**: AWS EC2 1대 (애플리케이션 + Redis 컨테이너), AWS RDS MySQL 8
- **배포 방식**: Docker Compose — `backend` · `frontend` · `redis` · `migrate` 4개 컨테이너
- **웹 서버**: frontend 컨테이너의 Nginx가 정적 파일을 서빙하고 `/api` · `/ws` · `/uploads` 요청을 `backend:8080`으로 프록시
- **DB 마이그레이션**: `migrate` 컨테이너가 Flyway로 RDS 스키마를 적용한 뒤 종료
- **이미지 저장**: EC2 로컬 `uploads/` 디렉터리 (S3 미연동)
- **CI/CD**: GitHub Actions — `dev` 브랜치 push 시 EC2 SSH 접속 → 최신 코드 pull → Docker Compose 재빌드·재배포 자동화
